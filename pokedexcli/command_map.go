...
func commandMap(config *Config, cache *internal.Cache, pokedex map[string]Pokemon, parameters []string) error {
    if config.Next == "" {
        err := errors.New("There are no more pages of location areas")
        return err
    }
    url := config.Next
